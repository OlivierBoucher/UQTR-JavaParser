...
import com.olivierboucher.inf1018.custom.*;
import java.io.*;
import java.util.*;
...
public class JavaParser implements JavaParserConstants {
  ...
  public JavaParser(File file, JavaObjectManager objectManager)
  {
    try
    {
  ...
  public JavaObjectManager getObjectManager()
  {
  ...
  {
    //INFO(Olivier): Assert that args[0] is provided
    if (args.length != 1)
    { //NOTE(Olivier): This is strict, maybe we could use < 0
    ...
    //INFO(Olivier): Assert that the path exists
    if (!file.exists())
    {
      System.out.println("The provided path does not exist.");
      System.exit(1);
    }
    //INFO(Olivier): Check if it is a directory or a single file
    if (!file.isFile())
    {
        //NOTE(Olivier): We get everything from the folder
    ...
    {
      //NOTE(Olivier): A single file was passed
      fileList.add(file);
    }

      JavaObjectManager om = new JavaObjectManager();
      JavaParser parser;
      File f = null; //Current file
      ...
      //NOTE(Olivier): Output results if parsed correctly
      if(errors == 0)
      {
        JavaProjectInterpreter interpreter = new JavaProjectInterpreter(om);
        interpreter.debugTree();
        interpreter.outputProjectStatisticsToConsole();
      }
  }
...
/*
	NOTE(Olivier): Modified this to return the actual package name
*/
...
/*
	NOTE(Olivier): Modified this to return the actual import name
*/
...
/*
	NOTE(Olivier): Adding a new class
*/
  final public void ClassOrInterfaceDeclaration(int modifiers) throws ParseException {
    ...
    if(isInterface)
    {
      //NOTE(Olivier): We are in an interface, there's no implements
      ...
      JavaInterface i = new JavaInterface(nameToken.toString());
          ...
          //NOTE(Olivier): We add each interface it inherits from	
      for(int j = 0; j < extendsList.size(); j++)
      {
        i.addInterface(new JavaInterface(extendsList.get(j)));
      }
      //NOTE(Olivier): Pass to the object manager
      objectManager.addToCurrentObject(i);
    }
    else
    {
      //NOTE(Olivier): We are in a class, limited to extendsList(0) unless its null
      JavaClass c = new JavaClass(nameToken.toString());
      c.setModifiers(modifiers);
      if(extendsList.size() > 0)
      {
        c.setSuperClassName(extendsList.get(0));
      }
          //NOTE(Olivier): We add each interface	
...
/*
	NOTE(Olivier): Modified to return a list of string representing the extends
*/
...
/*
	NOTE(Olivier): Modified to return a list of string representing the implements
*/
...
/*
	NOTE(Olivier): Modified to add field declaration to current class
*/
  ...
  String name = "";
    ...
    while (true) {
      ...
      jj_consume_token(COMMA);
      VariableDeclarator();
      //NOTE(Olivier): Type and modifiers are already defined		
      a.setName(name);
      objectManager.addToCurrentObject(a);
    }
    jj_consume_token(SEMICOLON);
...
/*
	NOTE(Olivier): Modified to propagate variable name
*/
...
/*
	NOTE(Olivier): Modified to get variable name
*/
...
/*
	NOTE(Olivier): Modified to record the method and add it to the actual class
*/
...
/*
	NOTE(Olivier): Modified to return a JavaMethod instance
*/
...
/*
	NOTE(Olivier): Modified to return a list of JavaAttribute
*/
...
/*
	NOTE(Olivier): Modified to return a JavaAttribute instance
*/
...
/*
	NOTE(Olivier): Modified to record the constructor to the actual class
				   Modified the method to accept modifiers as parameter
*/
...
/*
	NOTE(Olivier): Modified to return type name
*/
...
/*
	NOTE(Olivier): Modified to return the actual class or interface name
*/
...
/*
	NOTE(Olivier): Modified to return the primitive name
*/
...
/*
	NOTE(Olivier): Modified to return the type string
*/
...
/*
	NOTE(Olivier): Modified to return the token string
*/
...
/*
	NOTE(Olivier): Modified to track direct method calls
*/
...
/*
	NOTE(Olivier): Modified to return the name as a String
*/
 ...
 */
  final public void RUNSIGNEDSHIFT() throws ParseException {
    if (getToken(1).kind == GT &&
                    ((JavaToken)getToken(1)).realKind == RUNSIGNEDSHIFT) {
  ...
  final public void RSIGNEDSHIFT() throws ParseException {
    if (getToken(1).kind == GT &&
                    ((JavaToken)getToken(1)).realKind == RSIGNEDSHIFT) {
